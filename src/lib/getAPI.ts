...
const LIVE_API_URL = process.env.NEXT_PUBLIC_API_URL + "/api/plumbers";
const DEV_API_URL = "http://localhost:8000/api/plumbers";
...
export const getByStateCity = async (
  ...
  try {
    ...
    const response = await apiClient.get(
      `/${state}/${city}/plumbers-contractors`,
      { params },
    );
    ...
    console.error("Error fetching plumbers data:", error);
    return null;
  }
};
